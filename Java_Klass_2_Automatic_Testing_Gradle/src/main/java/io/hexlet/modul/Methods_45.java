...
public class Methods_45 {
    ...
    private static int[] right(int[] numbers, int... values) {
        ...
        for (var value : values) {
            numbersWithoutValues =
                    ArrayUtils.removeAllOccurrences(numbersWithoutValues,
                            value);
        }
    ...
    private static int[] wrong3(int[] numbers, int... values) {
        var size = numbers.length;
        var indexFrom = size == 0 ? 0 : 1;
        var indexTo = size - 1 <= indexFrom ? indexFrom : size - 1;
        var numbersIncomplete = Arrays.copyOfRange(numbers, indexFrom,
                indexTo);
        var numbersWithoutValues = right(numbersIncomplete, values);
        if (size != 0) {
            numbersWithoutValues = ArrayUtils.addFirst(numbersWithoutValues
                    , numbers[0]);
            numbersWithoutValues = ArrayUtils.add(numbersWithoutValues,
                    numbers[size - 1]);
        }
        return numbersWithoutValues;
    }
